...
export async function backward_word_reduction_compound_word_aa_(
	...
	const { length } = word
	const valid_wordset_aa = []
	const begin_subword_aa = []
	...
	const starter_word_a = [
		'the',
		'for'
	]
	const sort_begin_subword_aa = sort(begin_subword_aa, (l, r)=>{
		return l.length === 1 && starter_word_a.indexOf(l[0].toLowerCase()) > -1 ? -1 : r.length === 1 && starter_word_a.indexOf(r[0].toLowerCase()) > -1 ? 1 : l[0].slice(l[0].length - 1).toLowerCase() === 's' && l[0].length === r[0].length + 1 ? 1 : r[0].slice(r[0].length - 1).toLowerCase() === 's' && r[0].length === l[0].length + 1 ? -1 : l[0].length > r[0].length ? -1 : l[0].length < r[0].length ? 1 : l[0] > r[0] ? -1 : 1
	})
	for (let idx = 0; idx < sort_begin_subword_aa.length; idx += 1) {
		...
		const rest_subword_aa = await backward_word_reduction_compound_word_aa_(subword__rest, compound_aspell)
		each(rest_subword_aa, (rest_word_a)=>valid_wordset_aa.push(subword_a.concat(...rest_word_a)))
		if (rest_subword_aa.length) {
			...
			if (next_subword && (next_subword.slice(next_subword.length - 1) === 's' && subword.length + 1 === next_subword.length || subword.length === next_subword.length + 1)) {
				continue
			}
			break
		}
...
export {
	backward_word_reduction_compound_word_aa_ as _backward_word_export
}
